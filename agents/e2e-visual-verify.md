...
| `showTestPlan(page, title, items)` | **動画冒頭**: テスト名と検証項目一覧を □ チェックボックス付きで表示 |
| `showTestResults(page, title, results)` | **動画末尾**: 各検証項目の ✅/❌ 結果とサマリーを表示 |
...
/**
 * 動画冒頭: テスト名と検証項目一覧を □ チェックボックス付きで表示し、
 * 指定秒数後にフェードアウトで消す。録画に含まれるため、動画を見るだけでテスト内容がわかる。
 */
export async function showTestPlan(
    ...
    await page.evaluate(({ title, items }) => {
        ...
        list.style.cssText = 'max-width: 600px; width: 100%;';
        items.forEach(item => {
            ...
            checkbox.style.cssText = 'color: #888; font-size: 18px; min-width: 24px; text-align: center;';
...
test.describe(SCENARIO, () => {
    ...
    test(SCENARIO, async ({ page }) => {
        // === 動画冒頭: 検証項目一覧を □ 付きで表示 ===
        await showTestPlan(page, SCENARIO, checkItems);
        ...
        // === 動画末尾: 各検証項目の結果を表示 ===
        await showTestResults(page, SCENARIO, [
            { item: 'ログイン画面が表示される', passed: true },
            { item: 'メールアドレスが入力できる', passed: true },
            { item: 'パスワードが入力できる', passed: true },
            { item: 'ログインボタンが押せる', passed: true },
            { item: '正しい情報でダッシュボードに遷移する', passed: true },
            { item: '誤った情報でエラーメッセージが表示される', passed: false, note: '未実装' },
        ]);
    });
});
```
...
- **冒頭（showTestPlan）**: テスト開始前にフルスクリーンオーバーレイでシナリオ名・検証項目一覧を □ 付きで4秒間表示
- **末尾（showTestResults）**: テスト完了後にフルスクリーンオーバーレイで各検証項目の ✅/❌ 結果とサマリーを5秒間表示
